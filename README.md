...
- **未知传感器时钟偏移的在线估计**（`time_offset.py`）：v0.22/v0.23 都默认「时间戳可信」，只解决「何时融合」。车端的现实是接收机/相机各自跑晶振，**时间戳本身就是错的**——时钟快 `b` 秒，每条 fix 就比真实晚 `b` 秒，位姿以 `v·b` 系统性滞后（22 m/s × 120 ms = 2.64 m），**任何回卷-重放都救不回来**。本版把那部分假设也拿掉：把偏移作为状态量，与运动状态**联合在线估计**。
  - **测量模型（一行代数定全部）**：`h = p + v·(tau_hat - t)`，`tau_hat = t_rep - b`，故 `dh/db = -v`——**偏移只通过速度进入新息**，下面的可观性结论全部由此推出。
  - **可观性三档（本版核心）**：① **静止**：`dh/db=0`，多少帧都不缩 `P_bb`——**但同时也无害**（诱导误差 `v·b=0`）；② **匀速**：**仍不可观**，把时钟平移 `b`、轨迹起点平移 `v·b` 得到**逐位相同**的观测（测试里精确断言），所以「高速上匀速跑一段」这种最自然的标定手法信息量为零；③ **变速 + 独立参考时钟运动源**（轮速/IMU）：可观，且把初值位置边缘化后，`I(b)=Σ(v-v̄)²/σ²`，即**是速度的方差在提供信息，不是速度大小**。`bias_fisher_information()` / `crlb_bias_std()` 提供闭式界；demo 的城市场景（0→22 m/s、含机动、GNSS 1 m）界为 **3.9 ms**，估计器 `P_bb` 收敛到 **5.1 ms**，9 个种子散布 **8.5 ms**。
  - **两个结构性要求（已被测试钉死）**：① 滤波器时钟必须由**参考时钟通道**推进（`update_velocity`），`update()` 绝不推进时钟；② 偏移必须与参考时钟运动源同时存在才可辨识。
  - **踩坑（重要，值得记）**：初版让**被污染的 fix 流**推进时钟（`update` 里 `predict_to(t_rep)`），结果只在正偏移下收敛，负偏移一路滑到 `b_hat≈-0.25 s` **却报 5 ms 的 σ**（自信地错）。诊断用了三步：无噪声 (b, 起点位移) 代价网格 → 发现不是多峰而是**平坦谷**（`b ≥ b_true` 时代价恒为 0）；机制是 —— `b_hat` 偏大时参考速度帧全部落在时钟**之后**，而「滞后赋值的速度积分出滞后的轨迹」正好是「偏大的 b」所需要的，故错解自洽；谷是单侧的，所以表现出**符号不对称**。改由参考通道驱动时钟后谷消失，`[-0.3, +0.3] s` 全域对称准确。教训：**时钟语义属于整个估计器，不属于融合技巧**（与 v0.23 同一条的延伸）。
  - **另一个假设被证伪并记录**：曾怀疑轮速驱动的位置积分（左黎曼，滞后半拍）会伪造成 `dt/2` 的时钟偏移。无噪声实测**精确**（误差 +0.1 ms）且**与轮速频率无关**（0.01/0.02/0.05 s）——因为位置帧一直把位置钉住，残差正比于加速度而非速度。加「修正」反而引入干净的 `+dt/2` 偏差，故**未入库**。
  - 效果：demo `examples/time_offset_demo.py`（90 s 城区：静止 15 s → 加速 → 慢速段 → 快速段 → 停车；20 Hz 轮速做参考钟、10 Hz GNSS σ=1 m、时钟偏 +120 ms）——三条管线同噪声实现：naive（信时间戳）位置 RMSE **1.551 m**、online **0.034 m**、oracle（已知 b）0.041 m；**naive/online = 45×**；偏移估计 117.4 ms（真值 120，自身 σ 5.1 ms）。起始静止段估计平稳不收敛（不可观），起步后收敛——「不可观期恰是无害期」在图上可见。图 `examples/time_offset_demo.png`（所绘为 9 种子**中位数**那一跑，不是最好的一跑）。
  - 单元测试 +76（→ 全量 **395 通过**）；验证文档 `docs/v0.24.0-time-offset-validation.md`。
...
- [x] ~~未知传感器时钟偏移的在线估计（可观性 + 联合状态估计）~~ ✅ v0.24.0（`time_offset.py`：`ClockOffset`/`OffsetAugmentedFilter`/`bias_fisher_information`/`crlb_bias_std`；偏移只通过速度进入新息，静止与匀速均不可观、变速+参考时钟运动源可观，`σ_b → σ/√Σ(v-v̄)²`；时钟由参考通道推进，`update()` 不推进时钟）
